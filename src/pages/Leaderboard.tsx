...
function RowSkeleton() {
  return (
    <div className="flex items-center gap-4 px-5 py-4 animate-pulse border-b border-slate-100 last:border-0">
...
function MobileRankCard({ team, rank }: { team: any; rank: number }) {
  ...
  return (
    <div
      className={`rounded-xl border ${isTop3 ? style.border : "border-slate-200"} bg-white overflow-hidden ${isTop3 ? "shadow-sm" : ""} hover:shadow-md transition-shadow duration-200`}
    >
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        <div className="flex-1 min-w-0">
          <p className={`font-bold truncate ${isTop3 ? "text-slate-900 text-base" : "text-slate-800 text-sm"}`}>
            {team.name}
          </p>
          {team.college && (
            <p className="text-xs text-slate-500 truncate mt-0.5">{team.college}</p>
          )}
        </div>

        {/* Track chip */}
        {team.track && (
          <span className="text-xs font-semibold uppercase tracking-wide bg-purple-50 text-purple-700 border border-purple-200 px-2 py-0.5 rounded flex-shrink-0">
            {team.track}
          </span>
        )}
      </div>
...
function TableRow({ team, rank }: { team: any; rank: number }) {
  ...
  return (
    <tr className={`border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors ${isTop3 ? "bg-white" : ""}`}>
      {/* Rank */}
      ...
      <td className="px-4 py-4">
        <p className={`font-bold ${isTop3 ? "text-slate-900 text-base" : "text-slate-700 text-sm"}`}>
          {team.name}
        </p>
      </td>
      ...
      <td className="px-4 py-4">
        {team.track ? (
          <span className="text-xs font-semibold uppercase tracking-wide bg-purple-50 text-purple-700 border border-purple-200 px-2 py-0.5 rounded">
            {team.track}
          </span>
        ) : (
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-2">Results</h1>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-900 font-semibold text-lg">Results not yet announced</p>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    <th className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-slate-400 w-16">
                      Rank
                    </th>
